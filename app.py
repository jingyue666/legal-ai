...
class HunyuanClient:
    ...
    def _build_messages_with_history(self, system_prompt: str, history: List[Dict]) -> List[Dict]:
        """构建包含历史记录的消息列表，确保格式正确且最后一条是user"""
        messages = []
        ...
        if messages and messages[-1]["Role"] != "user":
            # 如果最后一条不是user，添加一个默认消息
            messages.append({"Role": "user", "Content": "请继续回答"})
        
        # 确保消息不为空
        if len(messages) <= 1:  # 只有system或连system都没有
    ...
    def chat(self, prompt, system_prompt):
        ...
        try:
            ...
            # 使用统一的消息构建方法
            messages = self._build_messages(enhanced_system_prompt, prompt)
    ...
    def chat_with_history(self, messages: List[Dict], system_prompt: str) -> str:
        ...
        try:
            ...
            # 使用统一的消息构建方法
            full_messages = self._build_messages_with_history(enhanced_system_prompt, messages)
            ...
            # 如果格式错误，尝试降级处理
            if "InvalidParameter" in error_msg or "Messages" in error_msg:
                try:
                    # 降级方案：只发送当前问题
                    if latest_user_msg:
                        return self.chat(latest_user_msg, system_prompt)
                except:
                    pass
...
请按照劳动仲裁申请书格式生成。"""
            ...
            messages = self._build_messages(
                "你是专业的法律文书生成助手，请严格按照法律文书格式生成规范、严谨的法律文书。",
                prompt
            )
...
请确保信息准确、实用。"""
            ...
            messages = self._build_messages(
                "你是专业的法律流程助手，请结合国家法律法规，分析案件的关键节点、法律时效、流程提醒。回答要清晰易懂，标注重要时间节点。",
                prompt
            )
